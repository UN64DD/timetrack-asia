...
export default function AdminDashboard() {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      // Fetch all events for admin/root
  ...
  const handleSaveEvent = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error && (error.code === '42703' || error.message.includes('variants') || error.message.includes('price_max'))) {
        ...
        error = retryError;
        if (!error) showNotification('Event Registry Updated (Legacy Mode)', 'warning');
      }
            ...
            {activeTab === 'overview' && (
              <motion.div key="overview" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
                <div className="grid grid-cols-4 gap-4">
                  <StatCard label="Live Events" value={eventCount.toString()} trend="Global" icon={<Calendar className="text-red-500" />} />
                  <StatCard label="Total Users" value={users.length.toString()} trend="Registry" icon={<Users className="text-red-500" />} />
                  <StatCard label="Total Regs" value={registrations.length.toString()} trend="Volume" icon={<Activity className="text-red-500" />} />
                  <StatCard label="System Uptime" value="99.9%" trend="Status" icon={<ShieldCheck className="text-red-500" />} />
                </div>
              </motion.div>
            )}
