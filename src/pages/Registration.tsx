...
export default function Registration() {
  ...
  const fetchEvent = async () => {
    try {
      // Connect to the public endpoint that was created earlier
      const res = await axios.get(`${API_URL}/events/public/${id}`);
      setEvent(res.data);
    } catch (err) {
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      const response = await axios.post(`${API_URL}/registrations/checkout`, payload, { headers });
      
      alert(`Payment Gate Protocol Initiated!\nRegistration ID: ${response.data.registration_id}\nTotal Amount: RM${response.data.total_amount}.00`);
      navigate('/dashboard');
    } catch (error: any) {
            ...
            <div className="space-y-10 text-center py-10">
              ...
              <div className="flex flex-col md:flex-row items-center justify-center gap-6 pt-10">
                ...
                <motion.button
                  ...
                  disabled={loading}
                  className="bg-brand text-black py-6 px-16 rounded-2xl font-black uppercase tracking-widest text-sm flex items-center gap-3 group shadow-[0_0_40px_rgba(204,255,0,0.4)] disabled:opacity-50"
                >
                  {loading ? 'Processing...' : `Authorize RM${variant ? variant.price : event.price}.00`}
                </motion.button>
              </div>
            </div>
          )}
